...
class Quarter(db.Model):
    ...
    # foreign keys
    cemetery_id = db.Column(db.Integer, db.ForeignKey(
        'cemetery.id'), nullable=False)
    tombstone_id = db.Column(
        db.Integer, db.ForeignKey('tombstone.id'), nullable=True)
...
class Cemetery(db.Model):
    ...
    def __repr__(self):
        return 'Cmentarz w woj. %s, powiat %s, miejscowość %s, ulica %s ' % (format(self.voivodeship), format(self.county), format(self.locality), format(self.street) )
...
class Outfit(db.Model):
    ...
    def __repr__(self):
        return '%s, marki  %s, rozmiar %s, w kolorze %s' % (format(self.type_of_clothing), format(self.brand), format(self.size), format(self.color) )
...
class Buried(db.Model):
    ...
    container_id = db.Column(
        db.Integer, ForeignKey('container.id'), nullable=True)
    quarter_id = db.Column(db.Integer, ForeignKey(
        'quarter.id'), nullable=False)
    funeral_id = db.Column(db.Integer, ForeignKey(
        'funeral.id'), nullable=False)
...
priest_temple = db.Table('priest_temple',
                         db.Column('priest_id', db.Integer(), db.ForeignKey(
                             'priest.id'), primary_key=True),
                         db.Column('temple_id', db.Integer(), db.ForeignKey(
                             'temple.id'), primary_key=True)
                         )
...
class Temple(db.Model):
    ...
    def __repr__(self):
        return 'Swiątynia wyznania {}, umiejscowiona w {}, {}, {}' % (format(self.religion), format(self.voivodeship), format(self.county), format(self.locality))
